    ...
    Loader.prototype.includeFilabOrgMenu = function includeFilabOrgMenu() {
        ...
        if (organizations.length > 1) {
            ...
            for (var i = 0; i < organizations.length; i++) {
                if (organizations[i].name != USERPROFILE.getCurrentOrganization()) {
                    ...
                    if (organizations[i].name == USERPROFILE.getUsername()) {
                        ...
                        a.prepend($('<img></img>').attr('src', '/store/static/oil/img/user.png').css('width', '20px'));
                        ul.prepend(li);
                    } else {
                        a.prepend($('<img></img>').attr('src', '/store/static/oil/img/group.png').css('width', '20px'));
                        ul.append(li);
                    }
            ...
            if (USERPROFILE.getCurrentOrganization() == USERPROFILE.getUsername()) {
                $('.oil-usr-icon').attr('src', '/store/static/oil/img/user.png');
                userElem.text(USERPROFILE.getCompleteName());
            } else {
                $('.oil-usr-icon').attr('src', '/store/static/oil/img/group.png');
